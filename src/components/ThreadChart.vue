<template>
  <div class="thread-chart" ref="containerEl"
    @mousemove="onHover" @mouseleave="hover.visible = false">

    <svg v-if="props.chartData.length"
      class="chart-svg"
      ...
      <line v-for="g in svgGrid" :key="g.y"
        :x1="PL" :y1="g.y" :x2="svgW - PR" :y2="g.y" class="cg-grid" />

      <!-- Zero line -->
      <line v-if="cRange.hasZero"
        :x1="PL" :y1="cY(0)" :x2="svgW - PR" :y2="cY(0)" class="cg-zero" />
      ...
      <g v-for="ch in activeChannels" :key="ch">
        ...
        <rect v-for="(d, i) in props.chartData" :key="i"
          :x="cX(i) - dotSz(i) / 2"
          :y="cY(d[ch]) - dotSz(i) / 2"
          :width="dotSz(i)" :height="dotSz(i)"
          :class="`cg-dot cg-dot-${ch}`"
          :style="hover.visible && hover.step === i ? 'stroke:#fff;stroke-width:1.5' : ''" />
      </g>

      <!-- Current scrubber step marker (dashed) -->
      <line v-if="props.chartData.length > 1"
        :x1="cX(clampStep)" :y1="PT"
        :x2="cX(clampStep)" :y2="PT + CH"
        class="cg-step" />

      <!-- Hover step marker -->
      <line v-if="hover.visible"
        :x1="cX(hover.step)" :y1="PT"
        :x2="cX(hover.step)" :y2="PT + CH"
        class="cg-hover-line" />

      <!-- Y-axis labels -->
      <text v-for="g in svgGrid" :key="`l${g.y}`"
        :x="PL - 4" :y="g.y"
        class="cg-label" text-anchor="end" dominant-baseline="middle">
        {{ g.label }}
      </text>

      <!-- X-axis labels -->
      <text :x="PL" :y="svgH - 3" class="cg-label" text-anchor="middle">0</text>
      <text v-if="props.chartData.length > 1"
        :x="svgW - PR" :y="svgH - 3" class="cg-label" text-anchor="end">
        {{ props.chartData.length - 1 }}
      </text>

      <!-- Legend -->
      <g v-for="(ch, ci) in activeChannels" :key="`leg${ch}`"
        :transform="`translate(${PL + ci * 34}, 2)`">
    ...
    <!-- Hover tooltip -->
    <div v-if="hover.visible && props.chartData.length"
      class="chart-tooltip"
      :style="{ left: hover.tooltipX + 'px' }">
      <div class="ct-step">step {{ hover.step }}</div>
      <div v-for="ch in activeChannels" :key="ch"
        class="ct-val" :style="{ color: CH_COLOR[ch] }">
...
/* ── Lines ── */
.cg-grid       { stroke: #2a2a2a; stroke-width: 0.5; }
.cg-zero       { stroke: #3a3a3a; stroke-width: 0.5; stroke-dasharray: 3 3; }
.cg-line       { stroke-width: 1.5; }
.cg-line-r     { stroke: #5dade2; }
.cg-line-g     { stroke: #5dbb5d; }
.cg-line-b     { stroke: #e8a838; }
.cg-step       { stroke: rgba(255,255,255,.2);  stroke-width: 0.5; stroke-dasharray: 2.5 2.5; }
.cg-hover-line { stroke: rgba(255,255,255,.55); stroke-width: 0.7; }
...
.cg-label { fill: #666; font-size: 10px; }
...
.ct-step { font-size: 9px;  color: #555; margin-bottom: 2px; }
.ct-val  { font-size: 10px; line-height: 1.6; }
